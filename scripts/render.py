...
CSS = """\
  ...
  table.summary { border-collapse: collapse; margin: .25rem 0 1rem; min-width: 320px; }
  ...
  .finding > header { display: flex; flex-wrap: wrap; gap: .5rem; align-items: baseline; margin-bottom: .2rem; }
  .finding h4 { margin: 0; font-size: .98rem; font-weight: 600; }
  .finding h4 .num { color: var(--muted); margin-right: .4rem; font-weight: 500; }
  .loc { display: flex; flex-wrap: wrap; gap: .5rem; align-items: baseline; margin-bottom: .4rem; }
  ...
  .conf, .unresolved { display: inline-block; padding: .08rem .45rem; border: 1px solid var(--rule); border-radius: 3px; font-size: .72rem; color: var(--muted); }
  .unresolved { border-color: var(--c-high); color: var(--c-high); }
...
def render_finding(f: dict, num: int) -> str:
    ...
    heading = f.get("title") or loc
    conf = (
        f'\n    <span class="conf">{esc(f["confidence"])} confidence</span>'
        if f["confidence"] != "high"
        else ""
    )
    # merge.py sets resolved=False when the cited file or line does not exist in
    # the worktree — a strong hint the location was guessed.
    unresolved = (
        '\n    <span class="unresolved">location unverified</span>'
...
<article class="finding {sev}" id="f{num}">
  <header>
    ...
    <h4><span class="num">{num}.</span>{esc(heading)}</h4>
  </header>
  <div class="loc">
    <code class="path">{esc(loc)}</code>
    <span class="dim">{esc(", ".join(f["dimensions"]))}</span>{conf}{unresolved}
  </div>
...
def main() -> None:
    ...
    # Confidence sorts ahead of file/line: within a severity, what the reviewer
    # was surest about should be read first.
    findings.sort(
        key=lambda f: (
    ...
    confident = {
        s: sum(1 for f in findings if f["severity"] == s and f["confidence"] == "high")
        for s in SEVERITIES
    }
    total = len(findings)
    ...
    merged_line = (
        ...
        else "NOT deduplicated — synthesis failed, findings are as reported"
    )

    revisions = ""
    if meta.get("headSha"):
        base = f"{meta.get('base') or 'base'} {meta['baseSha'][:9]}" if meta.get("baseSha") else ""
        arrow = f"{base} → " if base else ""
        revisions = f'\n  <div><strong>Revision:</strong> {esc(arrow)}HEAD {esc(meta["headSha"][:9])}</div>'
    ...
    summary_rows = "\n".join(
        f'    <tr><td>{s}</td><td class="n">{counts[s]}</td><td class="n">{confident[s]}</td></tr>'
        for s in SEVERITIES
    )
...
<p class="note">Every finding below is an unverified claim. Check each against the current code before acting on it.</p>
<div class="meta">
  <div><strong>Generated:</strong> {esc(meta["generated"])}</div>
  <div><strong>Scope:</strong> {esc(meta["scope"])}</div>{revisions}
  <div><strong>Agents:</strong> {esc(agents_line)}</div>
</div>

<h2>Summary</h2>
<table class="summary">
  <thead><tr><th>Severity</th><th>Count</th><th>High confidence</th></tr></thead>
  <tbody>
{summary_rows}
    <tr class="total"><td>total</td><td class="n">{total}</td><td class="n">{sum(confident.values())}</td></tr>
  </tbody>
</table>
...
<section class="coverage">
  ...
  <p><strong>Merge:</strong> {esc(merged_line)}</p>
</section>
</body>
</html>
"""
